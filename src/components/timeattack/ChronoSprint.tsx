...
export const ChronoSprint: React.FC<ChronoSprintProps> = ({ games, onBackToHub, onOpenLeaderboard }) => {
  ...
  const generateQuestion = useCallback((): Question | null => {
    ...
    if (rand < 0.45) {
      ...
      return {
        type: 'first',
        prompt: t('timeattack.chrono.promptWhichFirst'),
        badge: t('timeattack.chrono.badgeDuel'),
        choices,
      };
    }
    ...
    if (rand < 0.80) {
      ...
      const choices = [
        {
          label: t('timeattack.chrono.labelBefore', { title: anchor.title }),
          isCorrect: isBefore,
        },
        {
          label: t('timeattack.chrono.labelAfter', { title: anchor.title }),
          isCorrect: !isBefore,
        },
      ];
      ...
      return {
        type: 'before_after',
        prompt: t('timeattack.chrono.promptBeforeAfter', {
          title: target.title,
          year: `${anchor.title} (${anchor.releaseYear})`,
        }),
        badge: t('timeattack.chrono.badgeBeforeAfter'),
        anchorGame: anchor,
        choices,
      };
    }
    ...
    return {
      type: 'exact_year',
      prompt: t('timeattack.chrono.promptWhichGameYear', { year: targetGame.releaseYear }),
      badge: t('timeattack.chrono.badgeExactYear'),
  ...
  const shareCardData: ShareCardData = {
    gameMode: `${t('timeattack.chrono.title')} ⚡`,
    ...
    details: [
      ...
      `⚡ ${t('timeattack.share.chronoSprint')}`,
    ],
  };
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-mono">
              {t('timeattack.bestRecord')} : <span className="text-yellow-400 font-bold">{stats.highScore} pts</span>
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-[#0b0f19] border border-[#1e293b] rounded-2xl p-6 sm:p-8 text-center space-y-6 shadow-xl">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 max-w-lg mx-auto text-left">
            <div className="p-3 rounded-xl bg-[#131a29]/80 border border-[#1e293b]">
              <div className="text-[10px] text-slate-400 font-bold uppercase">{t('timeattack.hud.time')}</div>
              <div className="text-lg font-black text-yellow-400">60s</div>
            </div>
            <div className="p-3 rounded-xl bg-[#131a29]/80 border border-[#1e293b]">
              <div className="text-[10px] text-slate-400 font-bold uppercase">{t('timeattack.hud.correctLong')}</div>
              <div className="text-lg font-black text-emerald-400">+3s & +100pts</div>
            </div>
            <div className="p-3 rounded-xl bg-[#131a29]/80 border border-[#1e293b]">
              <div className="text-[10px] text-slate-400 font-bold uppercase">{t('timeattack.hud.wrong')}</div>
          ...
          >
            ...
            <span>{t('timeattack.hud.startSprintBtn')}</span>
          </button>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between p-3.5 bg-[#0b0f19] border border-[#1e293b] rounded-2xl shadow-lg">
            ...
            <div className="flex items-center gap-2 relative">
              ...
              <div>
                <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{t('timeattack.hud.time')}</div>
            ...
            <div className="flex items-center gap-4">
              <div className="text-right">
                <div className="text-[10px] text-slate-400 font-bold uppercase">{t('timeattack.hud.score')}</div>
          ...
          <div className="bg-[#0b0f19] border border-[#1e293b] rounded-2xl p-5 sm:p-6 space-y-5 shadow-xl relative overflow-hidden">
            <div className="flex items-center justify-between gap-2">
              ...
              <button
                ...
                title={t('timeattack.hud.skipPenalty')}
              >
        ...
        <div className="bg-[#0b0f19] border border-[#1e293b] rounded-2xl p-6 sm:p-8 text-center space-y-6 shadow-2xl animate-in zoom-in-95 duration-300">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-400">
              {t('timeattack.results.breakdown')}
            </p>
          </div>

          <div className="p-5 rounded-2xl bg-[#131a29]/90 border border-yellow-500/30 max-w-sm mx-auto">
            <div className="text-xs font-bold text-slate-400 uppercase">{t('timeattack.results.finalScore')}</div>
          ...
          <div className="grid grid-cols-3 gap-2.5 max-w-md mx-auto">
            <div className="p-3 rounded-xl bg-[#131a29]/60 border border-[#1e293b]">
              <div className="text-[10px] text-slate-400 font-bold uppercase">{t('timeattack.results.accuracy')}</div>
              <div className="text-base font-black text-emerald-400">
                {totalCount > 0 ? Math.round((correctCount / totalCount) * 100) : 0}%
              </div>
            </div>
            <div className="p-3 rounded-xl bg-[#131a29]/60 border border-[#1e293b]">
              <div className="text-[10px] text-slate-400 font-bold uppercase">{t('timeattack.results.solved')}</div>
              <div className="text-base font-black text-white">{correctCount}/{totalCount}</div>
            </div>
            <div className="p-3 rounded-xl bg-[#131a29]/60 border border-[#1e293b]">
              <div className="text-[10px] text-slate-400 font-bold uppercase">{t('timeattack.results.maxCombo')}</div>
