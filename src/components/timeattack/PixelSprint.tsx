...
export const PixelSprint: React.FC<PixelSprintProps> = ({ games, onBackToHub, onOpenLeaderboard }) => {
  ...
  const shareCardData: ShareCardData = {
    gameMode: `${t('timeattack.pixel.title')} ⚡`,
    date: new Date().toISOString().slice(0, 10),
    isWon: score > 0,
    scoreText: `${score} pts`,
    details: [
      `🎯 ${t('timeattack.share.pixelSolved', { correct: correctCount, total: totalCount })}`,
      `🔥 ${t('timeattack.results.maxCombo')} : x${maxCombo}`,
      `⚡ ${t('timeattack.share.pixelSprint')}`,
    ],
  };
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-mono">
              {t('timeattack.bestRecord')} : <span className="text-cyan-400 font-bold">{stats.highScore} pts</span>
            </p>
          </div>
        </div>
      </div>
