...
export const IndledleSprint: React.FC<IndledleSprintProps> = ({ games, onBackToHub, onOpenLeaderboard }) => {
  ...
  const generateQuestion = useCallback((): Question | null => {
    ...
    for (let attempt = 0; attempt < 10; attempt++) {
      ...
      if (qType === 'dev_of_game') {
        ...
        return {
          prompt: t('timeattack.indledle.promptDev', { title: randomGame.title }),
          subPrompt: t('timeattack.indledle.subDev', {
            year: randomGame.releaseYear,
            style: getLocalizedText(randomGame.artStyle, i18n.language),
          }),
          choices,
          correctChoice,
        };
      ...
      if (qType === 'game_of_dev') {
        ...
        return {
          prompt: t('timeattack.indledle.promptGame', { dev: randomGame.developer }),
          subPrompt: t('timeattack.indledle.subGame'),
          choices,
          correctChoice,
        };
      }
      ...
      if (qType === 'year_of_game') {
        ...
        return {
          prompt: t('timeattack.indledle.promptYear', { title: randomGame.title }),
          subPrompt: t('timeattack.indledle.subYear', { dev: randomGame.developer }),
          choices,
          correctChoice,
        };
      }
      ...
      return {
        prompt: t('timeattack.indledle.promptGenre', { title: randomGame.title }),
        subPrompt: t('timeattack.indledle.subGenre'),
        choices,
        correctChoice,
      };
    }
  ...
  const shareData: ShareCardData = {
    gameMode: t('timeattack.share.indledleSprint'),
    date: new Date().toLocaleDateString(i18n.language),
    isWon: score > 0,
    scoreText: `${score} pts • ${t('timeattack.share.correctCount', { correct: correctCount, total: totalCount })}`,
    details: [
      `${t('timeattack.hud.score')} : ${score} pts`,
      `${t('timeattack.results.solved')} : ${correctCount}/${totalCount}`,
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 max-w-xl mx-auto mb-8 text-left">
            <div className="p-3.5 rounded-2xl bg-[#131a29] border border-[#1e293b]">
              <div className="text-emerald-400 font-bold text-sm mb-0.5">+3s & {t('timeattack.combo')}</div>
              <p className="text-xs text-slate-400">{t('timeattack.profille.rule1')}</p>
            </div>
            <div className="p-3.5 rounded-2xl bg-[#131a29] border border-[#1e293b]">
              <div className="text-rose-400 font-bold text-sm mb-0.5">{t('timeattack.timePenalty')}</div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold text-sm">{getTranslatedGenre(choice, i18n.language)}</span>
                  </div>
      ...
      {gameState === 'gameover' && (
        ...
        >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 max-w-xl mx-auto mb-8">
            ...
            <div className="p-4 rounded-2xl bg-[#131a29] border border-[#1e293b]">
              <div className="text-[10px] uppercase font-bold text-slate-400 mb-1">{t('timeattack.results.correctCount')}</div>
