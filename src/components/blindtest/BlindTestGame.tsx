...
import {
  playBlindTestAudioClip,
  preloadBlindTestAudio,
  type PlayBlindTestHandle,
} from '../../utils/blindTestAudio';
...
export const BlindTestGame: React.FC<BlindTestGameProps> = ({ currentDate, onSelectDate }) => {
  ...
  const [isOfficialClip, setIsOfficialClip] = useState<boolean>(false);

  // Audio references
  const audioCtxRef = useRef<AudioContext | null>(null);
  const masterGainRef = useRef<GainNode | null>(null);
  const playbackHandleRef = useRef<PlayBlindTestHandle | null>(null);
  ...
  useEffect(() => {
    if (puzzle?.audioUrl) {
      preloadBlindTestAudio(puzzle.audioUrl);
    }
  }, [puzzle?.audioUrl]);
  ...
  // Jouer l'extrait musical (OST authentique avec repli synthétiseur)
  const playAudio = async () => {
    ...
    const handle = await playBlindTestAudioClip({
      ctx,
      audioUrl: puzzle.audioUrl,
      audioConfig: puzzle.audioConfig,
      playDuration,
      masterGain,
    });
  ...
  // Filtrage intelligent pour autocomplétion (insensible aux accents, ponctuations, compositeur & indices)
  const filteredGames = useMemo(() => {
    ...
    const normQ = rawQ
      ...
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '');
    const compactQ = normQ.replace(/[^a-z0-9]/g, '');

    const candidates = INDIE_GAMES.filter((game) => !guesses.includes(game.id));
    ...
    const scored = candidates.map((game) => {
      ...
      const normDev = game.developer
        .toLowerCase()
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '');
      const compactDev = normDev.replace(/[^a-z0-9]/g, '');

      const composer = game.hints.composer || '';
      const normComp = composer
        .toLowerCase()
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '');
      const compactComp = normComp.replace(/[^a-z0-9]/g, '');

      const normTaglineFr = (game.hints.tagline?.fr || '')
        .toLowerCase()
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '');
      const normTaglineEn = (game.hints.tagline?.en || '')
        .toLowerCase()
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '');

      const normGenres = game.genre.map((g) =>
      ...
      if (normTitle === normQ || (compactQ.length >= 3 && compactTitle === compactQ)) {
        score = 100;
      }
      // Le titre commence par la requête
      else if (normTitle.startsWith(normQ) || (compactQ.length >= 2 && compactTitle.startsWith(compactQ))) {
        score = 90;
      }
      // Le titre contient la requête
      else if (normTitle.includes(normQ) || (compactQ.length >= 3 && compactTitle.includes(compactQ))) {
        score = 80;
      }
      // Compositeur correspond (gère "Louis F.", "LouisF", "Louis")
      else if (
        (normComp && (normComp.includes(normQ) || (compactQ.length >= 3 && compactComp.includes(compactQ)))) ||
        (game.id === 'balatro' && (normQ.includes('louis') || normQ.includes('poker')))
      ) {
        score = 70;
      }
      // Studio développeur
      else if (normDev.includes(normQ) || (compactQ.length >= 3 && compactDev.includes(compactQ))) {
        score = 60;
      }
      // Genre du jeu (ex: Cartes, Deckbuilder, Roguelike)
      else if (normGenres.some((g) => g.includes(normQ))) {
        score = 50;
      }
      // Mots-clés de la phrase d'accroche (indice 5, ex: poker)
      else if (normTaglineFr.includes(normQ) || normTaglineEn.includes(normQ)) {
        score = 40;
      }
  ...
  const shareCardData: ShareCardData = useMemo(() => {
    ...
    const scoreHeadline = isWon
      ? `${totalAttempts}/5 ${t('blindtest.tries')}`
      : t('blindtest.unsolved');

    return {
      gameMode: 'Blind Test OST',
      date: isPracticeMode ? t('blindtest.practice') : currentDate,
      isWon,
      scoreText: scoreHeadline,
      details: [
        t('blindtest.shareTitle'),
        `🔊 ${AUDIO_UNLOCK_DURATIONS[Math.min(totalAttempts - 1, 4)]}s / 18.0s`,
        blocksLine,
        isWon
          ? t('blindtest.shareWonStep', { count: totalAttempts })
          : t('blindtest.revealedGame', { title: puzzle.targetGame.title }),
      ],
    };
  }, [guesses, isWon, isPracticeMode, currentDate, puzzle.targetGame.title, t]);
    ...
    <div className="w-full max-w-2xl mx-auto px-3 sm:px-4 py-4 pb-36 sm:pb-48 min-h-[calc(100vh-140px)] space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-[#0b0f19] border border-[#1e293b] rounded-2xl p-3.5 sm:p-4 shadow-lg">
            ...
            <div className="flex items-center gap-2">
              ...
              {isPracticeMode && (
                <span className="px-2 py-0.5 rounded-full text-[10px] font-black uppercase bg-purple-500/20 text-purple-300 border border-purple-500/40">
                  {t('blindtest.practice')}
                </span>
              )}
            </div>
            <p className="text-xs text-slate-400">
              {t('blindtest.gameSubtitle')}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title={t('blindtest.unlimitedPracticeTip')}
          >
            <RotateCcw className="w-3.5 h-3.5" />
            <span className="hidden sm:inline">
              {isPracticeMode ? t('blindtest.restart') : t('blindtest.practice')}
            </span>
          </button>
        </div>
      </div>
        ...
        <div className="px-4 py-3 bg-[#181432]/70 border-b border-purple-500/20 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono font-bold text-slate-300">
              {isCompleted
                ? puzzle.trackTitle
                : t('blindtest.mysteryTrack')}
            </span>
            {isOfficialClip && (
              <span className="hidden sm:inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold bg-amber-500/15 text-amber-300 border border-amber-500/30">
                ✨ {t('blindtest.officialClip', 'OST Officielle')}
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title={isMuted ? t('blindtest.soundOn') : t('blindtest.soundOff')}
            >
            ...
            <span className="px-2 py-0.5 rounded-md bg-purple-950/80 text-purple-300 text-[11px] font-mono font-black border border-purple-700/40">
              {t('blindtest.secondsUnlocked', { duration: maxAllowedDuration })}
            </span>
          </div>
        </div>
      ...
      <div className="space-y-2">
        <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5 px-1">
          ...
          <span>{t('blindtest.unlockedClues')}</span>
        </h3>
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'ArrowDown') {
                    ...
                    e.preventDefault();
                    if (filteredGames.length > 0) {
                      const candidate = filteredGames[selectedIndex] || filteredGames[0];
                ...
                placeholder={t('blindtest.searchPlaceholder')}
                  ...
                  {filteredGames.map((game, idx) => (
                    <button
                      ...
                      className={`w-full px-3.5 py-2.5 text-left flex items-center gap-3 transition-all cursor-pointer ${
                        idx === selectedIndex ? 'bg-purple-500/20 text-white' : 'text-slate-300 hover:bg-[#131d36]'
                      }`}
                    >
                      <img
        ...
        >
          ...
          {isWon ? (
            <div className="space-y-1">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 text-xs font-black uppercase">
                ...
                <span>{t('blindtest.perfectPitch')}</span>
              </div>
              <h3 className="text-lg sm:text-xl font-black text-white">
                {puzzle.targetGame.title}
              </h3>
              <p className="text-xs text-slate-400">
                {t('blindtest.wonDetails', {
                  count: guesses.length,
                  track: puzzle.trackTitle,
                  composer: puzzle.composer,
                })}
              </p>
            </div>
          ) : (
            <div className="space-y-1">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-rose-500/20 border border-rose-500/40 text-rose-300 text-xs font-black uppercase">
                ...
                <span>{t('blindtest.roundOver')}</span>
              </div>
              <h3 className="text-lg sm:text-xl font-black text-white">
                {puzzle.targetGame.title}
              </h3>
              <p className="text-xs text-slate-400">
                {t('blindtest.lostDetails', {
                  track: puzzle.trackTitle,
                  composer: puzzle.composer,
                })}
              </p>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 pt-1">
            ...
            >
              ...
              <span>{t('blindtest.shareResult')}</span>
            </button>
            ...
            >
              <RotateCcw className="w-3.5 h-3.5" />
              <span>{t('blindtest.playPracticeMode')}</span>
            </button>
          </div>
