...
export const PixelGame: React.FC<PixelGameProps> = ({ currentDate, onSelectDate }) => {
  ...
  const shareCardData: ShareCardData = useMemo(() => {
    ...
    return {
      ...
      details: [
        isFr ? '🧩 DÉ-PIXELLISATION & OMBRE' : '🧩 PIXEL & SILHOUETTE',
        blocksLine,
        isWon
          ? isFr
            ? `🎯 Résolu en ${totalAttempts} étape${totalAttempts > 1 ? 's' : ''} !`
            : `🎯 Solved in ${totalAttempts} step${totalAttempts > 1 ? 's' : ''}!`
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-3 sm:px-4 py-4 space-y-5">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-base sm:text-lg font-black text-white tracking-wide">
                {isFr ? 'Pixel & Silhouette' : 'Pixel & Silhouette'}
              </h2>
              {isPracticeMode && (
            ...
            <p className="text-xs text-slate-400">
              {isFr
                ? '5 essais pour identifier la pépite indé sous son voile rétro'
                : '5 guesses to identify the indie gem behind its retro veil'}
            </p>
          </div>
        </div>
      ...
      <div
        className={`relative bg-[#080d1a] border border-[#1e293b] rounded-2xl overflow-hidden shadow-2xl transition-transform ${
          ...
          {!isCompleted && (
            <div className="flex items-center bg-[#131b2e] p-0.5 rounded-lg border border-[#1e293b]">
              ...
              >
                ...
                <span>{isFr ? 'Silhouette' : 'Shadow'}</span>
              </button>
            </div>
          )}
        </div>
        ...
        <div className="relative w-full aspect-video flex items-center justify-center bg-slate-950 overflow-hidden">
          {!imageLoaded && !imageLoadError && (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-slate-950">
              ...
              <span className="text-xs text-slate-400 font-mono">
                {isFr ? 'Génération de la mosaïque...' : 'Generating mosaic...'}
              </span>
            </div>
          )}
          ...
          {imageLoadError && (
            <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center text-slate-400">
              ...
              <p className="text-xs">{isFr ? 'Image indisponible.' : 'Image unavailable.'}</p>
            </div>
          )}
          ...
          <div className="relative">
            <div className="relative flex items-center">
              ...
              <button
                ...
                title={isFr ? 'Dévoile plus de pixels sans deviner' : 'Unveil more pixels without guessing'}
              >
                {isFr ? 'Passer' : 'Skip'}
              </button>
            </div>
          ...
          {isWon ? (
            <div className="space-y-1">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 text-xs font-black uppercase">
                ...
                <span>{isFr ? 'Victoire Éclatante !' : 'Brilliant Victory!'}</span>
              </div>
              ...
              <p className="text-xs text-slate-400">
                {isFr
                  ? `Identifié avec succès en ${guesses.length} essai${guesses.length > 1 ? 's' : ''}.`
