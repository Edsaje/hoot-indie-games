...
export const ScreenleGame: React.FC<ScreenleGameProps> = ({ currentDate, onSelectDate }) => {
  ...
  // Suivi télémétrie cookieless au chargement du défi (sans divulgation de titre en clair dans l'onglet Réseau)
  useEffect(() => {
    telemetry.track('game', 'screenle_play', `session_${currentDate}`, undefined, {
      date: currentDate,
  ...
  // Zoom / crop styling based on stage & difficulty (Zoom amplifié sur les premières étapes)
  const zoomScalesMap: Record<GameDifficulty, number[]> = {
    novice: [2.6, 2.0, 1.6, 1.3, 1.15, 1.0],
    standard: [4.6, 3.4, 2.4, 1.8, 1.3, 1.0],
    expert: [6.5, 4.8, 3.5, 2.5, 1.6, 1.0],
  };
  const zoomOrigins = ['35% 40%', '65% 35%', '30% 65%', '70% 60%', '50% 50%', 'center center'];
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-3 sm:px-4 py-4 sm:py-8 pb-36 sm:pb-48 min-h-[calc(100vh-140px)] animate-in fade-in duration-300">
        ...
        {Array.from({ length: maxAttempts }).map((_, i) => {
          ...
          return (
            <button
              ...
              disabled={!isUnlocked}
              className={`flex items-center justify-center w-9 h-9 sm:w-10 sm:h-10 rounded-xl font-bold text-xs sm:text-sm transition-all touch-manipulation active:scale-95 ${
                isActive
      ...
      <div className="relative w-full aspect-video bg-[#06241b] border-2 border-[#78350f] rounded-2xl overflow-visible shadow-2xl group mb-6">
        <SylvestreIvyFrame density="delicate" />
        {/* The screenshot image with animated transform */}
        <div className="w-full h-full overflow-hidden rounded-2xl flex items-center justify-center bg-slate-950">
          <img
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            {!isTaglineUnlocked && !isCompleted && (
              <span className="text-xs ml-auto font-mono text-slate-400">
                {difficulty === 'expert' ? t('screenle.hiddenGrandDuc') : t('screenle.unlockHintAt', { step: 3 })}
              </span>
            )}
          </div>
          <p className="text-sm italic leading-relaxed text-slate-200">
            {isTaglineUnlocked
              ? `"${getLocalizedText(secretGame.hints?.tagline, i18n.language)}"`
              : difficulty === 'expert'
              ? t('screenle.noTextGrandDuc')
              : t('screenle.lockedStepTwo')}
          </p>
        </div>
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            ...
            {!isComposerUnlocked && !isCompleted && (
              <span className="text-xs ml-auto font-mono text-slate-400">
                {difficulty === 'expert' ? t('screenle.hiddenGrandDuc') : t('screenle.unlockHintAt', { step: 4 })}
              </span>
            )}
          </div>
          <p className="text-sm leading-relaxed text-slate-200">
            {isComposerUnlocked
              ? secretGame.hints.composer || t('screenle.composerIndie')
              : difficulty === 'expert'
              ? t('screenle.noAudioGrandDuc')
              : t('screenle.lockedStepThree')}
          </p>
        </div>
      </div>
