...
export const ChronoGame: React.FC<ChronoGameProps> = ({ currentDate, onSelectDate }) => {
  ...
  const handleInsertSlot = (slotIndex: number) => {
    ...
    if (isCorrect) {
      ...
      setFeedback({
        text: isFr
          ? `Exact ! ${currentCard.title} est sorti en ${targetYear}.`
          : `Correct! ${currentCard.title} released in ${targetYear}.`,
        isCorrect: true,
      });
      ...
      setFeedback({
        text: isFr
          ? `Manqué ! ${currentCard.title} est sorti en ${targetYear}.`
          : `Missed! ${currentCard.title} released in ${targetYear}.`,
        isCorrect: false,
      });
  ...
  const handleShare = () => {
    ...
    const resultText = isWon
      ? isFr
        ? `Victoire ! Frise complétée avec ${lives}/3 cœurs ${heartsEmoji}`
        : `Victory! Timeline completed with ${lives}/3 hearts ${heartsEmoji}`
      : isFr
        ? `Défaite... ${correctPlacements}/4 jeux bien placés`
        : `Defeat... ${correctPlacements}/4 games correctly slotted`;
  ...
  const shareData: ShareCardData = {
    ...
    date: currentDate,
    isWon,
    scoreText: isWon ? `${lives}/3 Cœurs restants` : `${correctPlacements}/4 Placés`,
    details: [
      `Date : #${currentDate}`,
      `Placements réussis : ${correctPlacements}/4`,
      `Vies restantes : ${lives}/3 ❤️`,
    ],
  };
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 py-6">
      ...
      <div className="text-center mb-6">
        ...
        <h1 className="text-2xl sm:text-4xl font-black text-white tracking-wide flex items-center justify-center gap-2">
          ...
          <span className="text-xs font-black uppercase px-2 py-0.5 rounded-md bg-amber-500/20 text-amber-300 border border-amber-500/30">
            {isPracticeMode ? (isFr ? 'Entraînement' : 'Practice') : (isFr ? 'Défi du Jour' : 'Daily')}
          </span>
        </h1>
        <p className="text-xs sm:text-sm text-slate-300 max-w-lg mx-auto mt-2 leading-relaxed">
          {isFr
            ? "Positionnez chaque pépite indé à sa place chronologique exacte dans l'Histoire du jeu vidéo !"
            : 'Slot each indie gem at its exact chronological position in gaming history!'}
        </p>
      ...
      {!isCompleted && currentCard && (
        ...
        >
          <div className="flex flex-col sm:flex-row items-center gap-4">
            ...
            <div className="flex-1 text-center sm:text-left">
              <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-amber-500/20 border border-amber-500/30 text-amber-300 text-[10px] font-black uppercase tracking-wider mb-1">
                ...
                <span>{isFr ? 'Carte à Placer' : 'Card to Slot'}</span>
              </div>
          ...
          <div className="mt-4 pt-3 border-t border-[#1e293b] text-center text-xs font-bold text-amber-300/90 flex items-center justify-center gap-1.5">
            ...
            <span>
              {isFr
                ? 'Cliquez sur un emplacement [+] ci-dessous pour insérer ce jeu dans la frise !'
                : 'Click an insertion slot [+] below to place this game on the timeline!'}
            </span>
          </div>
        </motion.div>
      )}
        ...
        {!isCompleted && (
          <div className="pl-12 sm:pl-16 relative">
            ...
            >
              ...
              <span>
                {isFr
                  ? `Placer avant ${placedCards[0]?.game.title || ''} (${placedCards[0]?.game.releaseYear || ''})`
                  : `Slot before ${placedCards[0]?.game.title || ''} (${placedCards[0]?.game.releaseYear || ''})`}
              </span>
            </button>
          </div>
        )}
              ...
              {isNextSlotAvailable && (
                <div className="pl-12 sm:pl-16 my-1.5 relative">
                  ...
                  >
                    ...
                    <span>
                      {isFr
                        ? `Entre ${item.game.releaseYear} et ${placedCards[idx + 1].game.releaseYear}`
                        : `Between ${item.game.releaseYear} and ${placedCards[idx + 1].game.releaseYear}`}
                    </span>
                  </button>
                </div>
              )}
        ...
        {!isCompleted && placedCards.length > 0 && (
          <div className="pl-12 sm:pl-16 relative pt-1">
            ...
            >
              ...
              <span>
                {isFr
                  ? `Placer après ${placedCards[placedCards.length - 1].game.title} (${placedCards[placedCards.length - 1].game.releaseYear})`
                  : `Slot after ${placedCards[placedCards.length - 1].game.title} (${placedCards[placedCards.length - 1].game.releaseYear})`}
              </span>
            </button>
          </div>
        )}
      ...
      {isCompleted && (
        <div className="mt-8 max-w-xl mx-auto p-5 sm:p-6 rounded-2xl bg-[#131a29] border border-amber-500/40 shadow-2xl animate-in fade-in zoom-in-95 duration-400">
              ...
              <div className="flex items-center justify-center sm:justify-start gap-2 mb-1">
                ...
                <div>
                  <h3 className="text-xl font-black text-white">
                    {isWon
                      ? isFr
                        ? 'Frise Complétée avec Succès !'
                        : 'Timeline Successfully Completed!'
                      : isFr
                        ? 'Vies Épuisées !'
                        : 'Out of Lives!'}
                  </h3>
                  <p className="text-xs text-slate-300">
                    {isWon
                      ? isFr
                        ? `Bravo ! Vous avez franchi le défi avec ${lives}/3 cœur(s) restant(s).`
                        : `Well done! You completed the challenge with ${lives}/3 heart(s) remaining.`
                      : isFr
                        ? `Vous avez placé correctement ${correctPlacements} jeu(x) sur 4.`
                        : `You correctly slotted ${correctPlacements} game(s) out of 4.`}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="mt-5 pt-4 border-t border-[#1e293b] flex flex-wrap items-center justify-between gap-3 text-xs">
            {isPracticeMode ? (
              ...
              >
                <span>{isFr ? '← Revenir au défi quotidien' : '← Return to daily challenge'}</span>
              </button>
            ) : (
              <span className="text-slate-400">
                {isFr ? 'Envie de rejouer sans attendre demain ?' : 'Want to keep playing right away?'}
              </span>
            )}
            ...
            >
              ...
              <span>{isFr ? 'Partie Entraînement Illimitée' : 'Unlimited Practice Game'}</span>
            </button>
          </div>
        </div>
      )}
